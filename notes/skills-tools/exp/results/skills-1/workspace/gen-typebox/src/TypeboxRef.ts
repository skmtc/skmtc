...
export class TypeboxRef extends TsSnippet {
  ...
  constructor({ context, refName, destinationPath, modifiers, rootRef, schema }: ConstructorProps) {
    ...
    if (context.modelDepth[`${typeboxEntry.id}:${refName}`] > 0) {
      // A back-reference to a model still open on the build stack. Reuse
      // the model's identity from its statics without constructing it —
      // the Driver higher up the stack is already building it.
