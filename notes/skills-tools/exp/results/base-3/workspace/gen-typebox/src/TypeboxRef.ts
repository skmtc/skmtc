...
export class TypeboxRef extends TsSnippet {
  ...
  constructor({ context, refName, destinationPath, modifiers, rootRef, schema }: ConstructorProps) {
    ...
    if (context.modelDepth[`${denoJson.name}:${refName}`] > 0) {
      // A back-reference to a model still open on the build stack. Emitting
      // the bare identifier here avoids infinite recursion; a truly cyclic
      // schema would need Type.Recursive, which restructures the enclosing
      // model and is out of scope for this generator.
      context.modelDepth[`${denoJson.name}:${refName}`]++
