...
import { TypeboxNumber, TypeboxInteger } from './TypeboxNumber.ts'
import { TypeboxBoolean } from './TypeboxBoolean.ts'
import { TypeboxUnknown, TypeboxVoid } from './TypeboxMisc.ts'
 ...
 * receive only decomposed parts (`items` / `members` / `refName`).
 */
export const toTypeboxValue: SchemaToValueFn = ({
  ...
  // to the `SchemaType` union lets the switch below narrow each case on its
  // own — generator code narrows, it does not assert.
