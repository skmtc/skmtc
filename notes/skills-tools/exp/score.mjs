#!/usr/bin/env node
// Scores an experiment run workspace. Usage: node score.mjs <workspaceDir>
...
const EXPECTED_MODELS = ['Order', 'OrderItem', 'OrderStatus', 'Address']
...
let lintStdout = ''
try {
  lintStdout = execFileSync('deno', ['lint', '--json', 'gen-typebox/'], {
    cwd: workspace,
    encoding: 'utf8',
    stdio: ['ignore', 'pipe', 'pipe'],
    timeout: 120_000
...
const genEval = run('node', [GEN_EVAL_CLI, 'gen-typebox', '--json', genEvalJson])
