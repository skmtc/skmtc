...
export class EffectSchemaObject extends TsSnippet {
  ...
  override toString(): string {
    ...
    // one schema. `Schema.extend` grafts the record's index signature
    // onto the struct's declared fields.
...
class EffectSchemaRecord extends TsSnippet {
  ...
  override toString(): string {
    // SLOT(record): string-keyed map of this.value. effect's
    // `Schema.Record` takes a single named-argument object.
