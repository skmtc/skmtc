/**
 * Engine gate for @exp/gen-effect-schema: the fixture below runs
 * through the REAL pipeline and the rendered artifacts are pinned
 * byte-for-byte. The pinned strings carry effect's syntax; the
 * structural assertions (files exist, shared refs dedup, imports
 * stitched, recursion broken by an explicit annotation) are the
 * skeleton's and must keep passing unchanged.
 *
...
Deno.test('self-recursion defers and annotates the thunk', () => {
  const { artifacts } = generate()

  const category = artifacts['models/Category.generated.ts']

  // SLOT(lazy) + SLOT(recursion-annotation): for effect both land in one
  // place — the explicit return type on the suspend thunk is what stops
  // inference re-entering the binding (TS7022), so the `export const`
  // itself stays unannotated and keeps its inferred field types.
  assertStringIncludes(
    category,
