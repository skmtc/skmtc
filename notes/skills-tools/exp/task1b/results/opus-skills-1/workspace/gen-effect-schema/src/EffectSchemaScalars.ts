...
export class EffectSchemaNumber extends TsSnippet {
  ...
  override toString(): string {
    // SLOT(number): effect's primitives are values, not calls.
    // minimum / maximum / multipleOf live on this.schema and would be
    // composed here via `.pipe(Schema.greaterThanOrEqualTo(n))`.
...
export class EffectSchemaInteger extends TsSnippet {
  ...
  override toString(): string {
    // SLOT(integer): effect ships the composed form as `Schema.Int`
    // (Number refined by the `int` filter).
...
export class EffectSchemaVoid extends TsSnippet {
  ...
  constructor({ context, generatorKey, destinationPath }: EffectSchemaVoidArgs) {
