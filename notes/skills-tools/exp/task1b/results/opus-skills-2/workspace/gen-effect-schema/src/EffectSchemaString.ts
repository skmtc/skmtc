...
export class EffectSchemaString extends TsSnippet {
  ...
  override toString(): string {
    ...
    // effect's `Schema.Literal` is variadic and covers both cases — one
    // member is a literal, many members are a literal union — so no
    // separate enum form is needed. A null member comes from
    // OpenAPI 3.1-style nullable enums and must not be quoted;
    // `Schema.Literal` accepts `null` as a member directly.
    const content = enums?.length
      ? `${LIB}.Literal(${enums.map(literal).join(', ')})`
      : `${LIB}.String`

    // SLOT(string-constraints): minLength / maxLength / pattern /
    // format live on this.stringSchema — effect appends those as
    // `.pipe(Schema.minLength(n))` filters.
