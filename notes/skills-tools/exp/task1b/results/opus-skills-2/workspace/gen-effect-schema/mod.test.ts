...
Deno.test('self-recursion renders lazy and breaks the type cycle', () => {
  const { artifacts } = generate()

  const category = artifacts['models/Category.generated.ts']

  // SLOT(lazy) — for effect the thunk's EXPLICIT return type is what
  // breaks TS7022, so it is part of the deferred form itself.
  assertStringIncludes(
    category,
    'Schema.suspend((): Schema.Schema<any> => Category)',
  )
  // SLOT(recursion-annotation) is empty by design for effect: the cycle
  // is already broken inside the thunk, so the binding stays inferred
  // rather than being widened to `Schema.Schema<any>`.
