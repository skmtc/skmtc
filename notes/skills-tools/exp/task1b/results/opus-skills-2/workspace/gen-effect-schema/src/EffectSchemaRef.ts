...
export class EffectSchemaRef extends TsSnippet {
  ...
  override toString(): string {
    // SLOT(lazy): effect defers a recursive reference with
    // `Schema.suspend`. The EXPLICIT return type on the thunk is what
    // makes the emitted file compile: it stops TypeScript from having
    // to infer the enclosing `export const` from an initializer that
    // references itself (TS7022/TS7024), so the binding itself needs no
    // annotation — see SLOT(recursion-annotation) in the projection.
    //
    // The suspend wrapper goes INSIDE the modifiers: effect's
    // `Schema.optional` yields a PropertySignature, which is not a
    // Schema and so cannot be the thunk's return value.
