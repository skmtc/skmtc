...
export class EffectSchemaUnion extends TsSnippet {
  ...
  override toString(): string {
    ...
    // SLOT(union): oneOf/anyOf syntax. effect has no dedicated
    // discriminated-union constructor — `Schema.Union` already narrows
    // on a literal tag property, so the discriminator needs no separate
    // form and is kept only as metadata for peers.
