#!/usr/bin/env node
// Scores a task1b run workspace. Usage: node score1b.mjs <workspaceDir>
...
const outDir = join(workspace, 'out', 'models')
...
for (const file of outFiles) {
  ...
  for (const match of text.matchAll(/export\s+(?:const|type|interface|class)\s+(\w+)/g)) {
...
// 4. cross-file imports present where refs are used (Order must import Address)
const orderFile = outFiles.find(file => file.toLowerCase().startsWith('order.'))
score.orderImportsAddress = orderFile
  ? /import[^;\n]*Address/.test(readFileSync(join(outDir, orderFile), 'utf8'))
  : false
...
let lintStdout = ''
try {
  lintStdout = execFileSync('deno', ['lint', '--json', 'gen-effect-schema/'], {
    cwd: workspace,
    encoding: 'utf8',
    stdio: ['ignore', 'pipe', 'pipe'],
    timeout: 120_000
...
const genEval = run('node', [GEN_EVAL_CLI, 'gen-effect-schema', '--json', genEvalJson])
...
const genDir = join(workspace, 'gen-effect-schema')
let slotMarkers = 0
let hasLibTs = false
...
score.categoryRecursion = categoryFile
  ? /suspend|lazy/i.test(readFileSync(join(outDir, categoryFile), 'utf8'))
  : false
...
console.log(`ref import:        ${flag(score.orderImportsAddress)}`)
