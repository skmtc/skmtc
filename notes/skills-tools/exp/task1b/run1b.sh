#!/bin/bash
# EXP-5 runner (task1b: exemplar-poor MODEL task, effect/Schema target).
# Usage: ./run1b.sh <arm> <run-id> [model]
#   arm: baseline | skills
...
ARM_SKILLS=(skmtc-generator-v3 skmtc-lang-typescript-v3 skmtc-model-v3)
...
`skmtc-generator-v3`, then `skmtc-lang-typescript-v3`, and follow them.
After each meaningful change, run `deno task verify` and read its output
before continuing.
EOF
    ;;
...
set +e
node "$EXP_DIR/score1b.mjs" "$WORK_DIR" | tee "$RESULTS_DIR/score-summary.txt"
set -e
...
echo "{\"arm\": \"$ARM\", \"runId\": \"$RUN_ID\", \"task\": \"task1b\", \"model\": \"$MODEL\", \"agentExit\": $AGENT_EXIT, \"durationSeconds\": $DURATION}" > "$RESULTS_DIR/meta.json"
