...
export const renderPull = async ({
  ...
  const confirmOverwrite = mode === "interactive" && !force
    ? (
      { account, slug, enrichmentGenerators }: {
        account: string;
        slug: string;
  ...
  switch (format) {
    ...
    case "text": {
      switch (result.type) {
        ...
        case "aborted": {
          console.log(
            `Pull aborted — local "${result.projectName}" left unchanged.`,
          );
          return;
        }
        case "failed": {
          console.error(
            `Pull failed for "${result.projectName}" at ${result.stage}:`,
          );
        ...
        default: {
          ...
          throw new Error(
            `Unhandled pull result: ${JSON.stringify(_exhaustive)}`,
          );
        }
      }
    }
