...
Deno.test('renderClone - interactive mode mounts the Ink App with the expected state', async () => {
...
Deno.test('printCloneResult - json format emits a parseable object with verifyWith', () => {
  ...
  assertEquals(parsed.cloned, [{ moduleName: '@skmtc/gen-typescript', version: '0.0.55' }])
...
Deno.test('renderClone - missing project name fails with a recipe error', async () => {
...
Deno.test('renderClone - missing --generator flag fails in strict mode', async () => {
...
Deno.test('renderClone - empty --generator list also fails', async () => {
  const { errors, exitCode } = await withCapturedExit(async () => {
    await renderClone({
      projectName: 'my-api',
      generators: [],
