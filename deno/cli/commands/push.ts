...
type RenderPushArgs = {
  ...
  baseFiles?: boolean
  /** `--base-files-only`: push only the app tree; leave the hub config untouched. */
  baseFilesOnly?: boolean
  jsonFlag?: boolean
  noInputFlag?: boolean
}

const USAGE = 'skmtc push <project> [--project @account/slug] [--base-files | --base-files-only]'
...
export const renderPush = async ({
  ...
  baseFilesOnly: baseFilesOnlyFlag,
  jsonFlag,
  noInputFlag
}: RenderPushArgs) => {
          ...
          Promise.resolve(
            confirm(
              `${account}/${slug} already has config ` +
                `(${enrichmentCount} enrichment row${enrichmentCount === 1 ? '' : 's'}). Overwrite?`
            )
          )
      : undefined

  // `--base-files-only` implies collecting the base files; it just skips the
  // client-config PUT downstream.
  const baseFiles =
    baseFilesFlag || baseFilesOnlyFlag
  ...
  switch (format) {
    ...
    case 'text': {
      switch (result.kind) {
        ...
        case 'failed': {
          console.error(`Push failed for "${result.projectName}" at ${result.stage}:`)
          console.error(`  ${result.reason}`)
          return
        }
        default: {
          const _exhaustive: never = result
          throw new Error(`Unhandled push result: ${JSON.stringify(_exhaustive)}`)
        }
      }
    }
    default: {
