...
function resolveFrom(
  ...
  return skmtcRoot.projects.length === 1
    ? skmtcRoot.projects[0].name
    : undefined;
}
...
const CREATE_USAGE =
  "skmtc project create <name> [--from <project>] [--base-files]";
...
export const printCreateResult = (
  ...
  switch (result.kind) {
    ...
    case "failed": {
      console.error(
        `Create failed for "${result.projectName}" at ${result.stage}:`,
      );
    ...
    default: {
      ...
      throw new Error(
        `Unhandled create result: ${JSON.stringify(_exhaustive)}`,
      );
    }
  }
};
...
export const printRmResult = (
  ...
  switch (result.kind) {
    ...
    case "failed": {
      console.error(
        `Remove failed for "${result.projectName}" at ${result.stage}:`,
      );
      console.error(`  ${result.reason}`);
      return;
    }
