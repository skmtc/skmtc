 ...
 * multipart `files` part each) alongside the `version` part in the single
 * atomic `POST /v1/stacks/{account}/{stack}/versions` request — source is
 * the deliverable (the hub deploys it directly; there is no bundle part); the hub writes the tree to R2, reconciles the stack's
 ...
 *
 *  - derived deploy/engine artefacts at the project root — `server.ts` /
 *    `server.js` (the hub synthesizes the server entry itself at deploy),
 *    plus `bundle.js` / `worker.ts` (regenerated by the engine).
 *    Root-anchored so a generator that legitimately ships a nested
 *    `worker.ts` isn't caught.
