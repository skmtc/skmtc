...
export const pushHeadless = async ({
  ...
  try {
    const getResponse = await fetch(`${origin}/v1/projects/${account}/${slug}/config`, {
      method: 'GET',
  ...
  if (!baseFilesOnly) {
    ...
    try {
      const putResponse = await fetch(`${origin}/v1/projects/${account}/${slug}/client-config`, {
        method: 'PUT',
        headers: {
      ...
      if (!putResponse.ok) {
        ...
        return {
          type: 'failed',
          projectName,
          reason: `client-config push failed (${putResponse.status})${hint}: ${text.slice(0, 500)}`,
          stage: 'push'
        }
      }
  ...
  if (baseFiles && Object.keys(baseFiles).length > 0) {
    try {
      ...
      const configNote = baseFilesOnly ? '' : ' — client-config was already updated'
      if (!filesResponse.ok) {
        const text = await filesResponse.text()
        return {
          type: 'failed',
          projectName,
          reason: `base-files push failed (${filesResponse.status})${configNote}: ${text.slice(
            0,
            500
          )}`,
          stage: 'push'
