...
const readPackageJson = (appRoot: string): Record<string, unknown> | undefined => {
  try {
    const parsed: unknown = JSON.parse(Deno.readTextFileSync(join(appRoot, 'package.json')))
...
export const detectFormatter = (appRoot: string): DetectedFormatter | undefined => {
  const packageJson = readPackageJson(appRoot)

  const prettierConfig = PRETTIER_CONFIGS.find(name => fileExists(join(appRoot, name)))
  ...
  const biomeConfig = ['biome.json', 'biome.jsonc'].find(name => fileExists(join(appRoot, name)))
  ...
  const denoConfig = ['deno.json', 'deno.jsonc'].find(name => fileExists(join(appRoot, name)))
