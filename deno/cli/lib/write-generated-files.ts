 ...
 */
export const toEjectedArtifactPaths = (
...
export const writeGeneratedFiles = ({
  ...
  const generatedSuffix = clientSettings?.generatedSuffix ??
    DEFAULT_GENERATED_SUFFIX;
  ...
  for (
    const [artifactPath, artifactContent] of Object.entries(artifacts ?? {})
  ) {
  ...
  for (
    const { artifactPath, absolutePath, canonicalHash, content }
      of pendingWrites
  ) {
  ...
  for (
    ...
    try {
      const onDisk = Deno.readTextFileSync(
        join(skmtcRootPath, "..", artifactPath),
      );
...
const toEjectionReport = ({
  ...
  for (const ejectedExportPath of ejectedExportPaths) {
    ...
    const matchesGenerated = diskContent !== undefined &&
      (formatterCommand
        ? formatContent({
          command: formatterCommand,
    ...
    const baselineContent = projectPath &&
        existsSync(toCommittedBaselinePath(projectPath, ownedArtifactPath))
      ? Deno.readTextFileSync(
        toCommittedBaselinePath(projectPath, ownedArtifactPath),
      )
      : undefined;
    ...
    const classification = baselineContent !== undefined &&
