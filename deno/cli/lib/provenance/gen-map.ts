// CANONICAL HOME of the sidecar decode pipeline (moved here from the hub
// extension's prototype copy, itself ported from `packages/vite/src/gen-map.ts`
// — the plugin does not
// export its reader, so the decode + manifest-guard + re-anchor pipeline is
// copy-forked here; keep in lockstep). Two adaptations: the wire types come
// from `./types.ts`, and the manifest reader (`readManifestFiles`, from the
// plugin's `artifacts.ts`) is inlined below.
//
// Decode the attribution sidecars from the last `--anchors` generate
// (`.skmtc/<project>/.maps/**/*.skm.json`) into flat gen-map entries.
// Malformed sidecars and out-of-range pool indices degrade to empty
// strings, never throw.

import { readFile, readdir } from 'node:fs/promises'
import { join } from 'node:path'
import { parseForReanchor, REANCHOR_PARSER_ID } from '@/lib/provenance/reanchor.ts'
import type { GenMapEntry, GenMapResult } from '@/lib/provenance/types.ts'
...
export const resolveAliasPath = (path: string, basePath: string): string => {
 ...
 *  project hasn't been generated. Membership doubles as the gen-map path
 *  guard; `characters` drives the span-alignment check. (Inlined from the
 *  plugin's `artifacts.ts`.) */
...
const reanchoredEntries = async (
  ...
  return sidecar.anchors.flatMap(anchor => {
    ...
    const outcome = parsed.reanchor(landmark, path)
    switch (outcome.type) {
      case 'resolved':
        return [rowToEntry(sidecar, artifactPath, anchor, outcome.span)]
      case 'landmark-missing':
      case 'path-broken':
        return []
      default: {
        const _exhaustive: never = outcome
        throw new Error(`Unhandled reanchor outcome: ${JSON.stringify(_exhaustive)}`)
      }
    }
  })
}
 ...
 * a manifest `files` key are included (`.maps` accumulates sidecars for
 * renamed/removed artifacts and older generates). Files whose on-disk length
 * differs from the manifest's `characters` (a formatter ran after generate)
 * are RE-ANCHORED against the current text; `staleFiles` holds only files
 * that can't be re-anchored at all.
 *
 * One sidecar wins per artifact: the sidecar at the canonical mirror path
 * (`.maps/<f>.skm.json`) is preferred; ties fall to the lexicographically
 * first path — deterministic either way.
 */
