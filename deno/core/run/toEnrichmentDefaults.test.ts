...
Deno.test('toEnrichmentDefaults', async t => {
  ...
  await t.step(
    'an operation generator seeds only its isSupported subjects, keyed [path][method].main',
    () => {
...
Deno.test('toEnrichmentDefaults - webhook arm', async t => {
  ...
  await t.step(
    'a webhook generator seeds only its isSupported webhooks, keyed [name][method].main',
    () => {
