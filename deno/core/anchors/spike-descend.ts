...
const trim = (text: string, from: number, to: number): { from: number; to: number } => {
  ...
  for (const [li, pi, , , , from, to] of sidecar.A) {
    ...
    if (normalize(formattedSlice).includes(normalize(rawSlice))) {
      ...
      if (tally.mismatched <= 5) {
        ...
        console.error(`  fmt: ${formattedSlice.slice(0, 90).replaceAll('\n', ' ')}`)
      }
    }
  }
}

const rate = tally.anchors === 0 ? 'n/a' : `${((tally.matched / tally.anchors) * 100).toFixed(2)}%`
