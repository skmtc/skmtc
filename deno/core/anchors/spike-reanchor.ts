...
const childrenOf = (node: OxcNodeLike): OxcNodeLike[] => {
  ...
  for (const key of Object.keys(node)) {
    if (
      key === "type" || key === "start" || key === "end" || key === "_parent"
    ) continue;
...
const descend = (
  landmark: OxcNodeLike,
  path: number[],
): OxcNodeLike | undefined => {
...
const collectAllTopLevel = (parsed: unknown): Map<string, NodeHandle> => {
  ...
  for (const statement of body) {
    if (statement.type === "VariableDeclaration") {
      const declarations =
        (statement.declarations as OxcNodeLike[] | undefined) ?? [];
...
const readSidecar = (raw: string): SidecarFile | undefined => {
  ...
  if (
    typeof record.f !== "string" || !Array.isArray(record.A) ||
  ...
  const landmarks = record.L.filter((entry): entry is string =>
    typeof entry === "string"
  );
...
if (!mapsDir || !rawRoot || !formattedRoot) {
  console.error(
    "usage: spike-reanchor.ts <mapsDir> <rawRoot> <formattedRoot> [limit]",
  );
  Deno.exit(2);
}
...
for (const sidecarPath of sidecarPaths.slice(0, limit)) {
  ...
  const artifactRelative = sidecar.f.startsWith("@/")
    ? sidecar.f.slice(2)
    : sidecar.f;
  let rawText: string;
  ...
  try {
    ...
    formattedText = await Deno.readTextFile(
      join(formattedRoot, artifactRelative),
    );
  } catch {
    continue;
  }
  ...
  for (const row of sidecar.A) {
    ...
    const rawNodeHandle: NodeHandle = oxcAdapter.smallestEnclosing(
      rawParsed,
      from,
      to,
    );
    ...
    if (location.landmark === "") {
      ...
      if (noLandmarkSamples.length < 8) {
        noLandmarkSamples.push(
          `${artifactRelative} [${from},${to}) ${
            (rawNodeHandle as OxcNodeLike).type
          }: ` +
    ...
    const formattedLandmark = formattedLandmarks.get(location.landmark) as
      | OxcNodeLike
      | undefined;
    if (formattedLandmark === undefined) {
      tally.landmarkMissing += 1;
      brokenSamples.push(
        `${artifactRelative}: landmark ${location.landmark} missing`,
      );
      continue;
    }
    ...
    const formattedSlice = formattedText.slice(
      formattedNode.start,
      formattedNode.end,
    );
    if (rawNode.type !== formattedNode.type) {
      ...
      if (mismatchSamples.length < 10) {
        mismatchSamples.push({
          ...
          landmark:
            `${location.landmark} (type ${rawNode.type} vs ${formattedNode.type})`,
