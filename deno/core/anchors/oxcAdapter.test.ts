...
Deno.test('collectLandmarks - indexes non-exported top-level declarations', () => {
  ...
  assertEquals([...landmarks.keys()].sort(), [
    'LocalType',
    'PublicType',
    'columnHelper',
    'columns',
...
Deno.test('paths survive a formatter JSX reflow (whitespace JSXText filtered)', () => {
  ...
  const node = oxcAdapter.smallestEnclosing(rawParsed, from, from + target.length)
  ...
  const formattedLandmark = oxcAdapter.collectLandmarks(formattedParsed).get('Widget')
  ...
  const formattedSlice = formatted.slice(reanchoredSpan.start, reanchoredSpan.end)
