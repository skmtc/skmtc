...
// dnt builds the npm `@skmtc/core` package into ../../packages/core. Each
// `entryPoints` item becomes a fine-grained npm SUBPATH export
// (`@skmtc/core/OasDocument`, `@skmtc/core/ParseContext`, …) — this is how
// browser/node consumers (e.g. skmtc-hub `apps/main` docs) import only the
// parse segment and let their bundler tree-shake the codegen engine away.
// Restored from git (deleted in e137ac7b) + `./OasWebhook` added for the 3.1
// webhook IR; `./PrettierConfig` dropped (file removed); `./OasComponents`
// de-duplicated.
...
await build({
  entryPoints: [
    ...
    { name: './GeneratorKeys', path: './dsl/GeneratorKeys.ts' },
    ...
    { name: './OasWebhook', path: './oas/webhook/Webhook.ts' },
