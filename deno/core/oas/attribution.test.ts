...
Deno.test('attribution - parsed schemas carry JSON-Pointer locations', () => {
  ...
  assertEquals(user.stackTrail.toJsonPointer(), '#/components/schemas/User')
  ...
  assertEquals(idProp.stackTrail.toJsonPointer(), '#/components/schemas/User/properties/id')
  ...
  assertEquals(ageProp.stackTrail.toJsonPointer(), '#/components/schemas/User/properties/age')

  const op = doc.operations[0]
  assertEquals(op?.stackTrail.toJsonPointer(), '#/paths/~1users~1{id}/get')

  const param = op?.parameters?.[0]
  if (!param || param.isRef()) throw new Error('expected concrete parameter')
  assertEquals(param.stackTrail.toJsonPointer(), '#/paths/~1users~1{id}/get/parameters/0')

  const response = op?.responses['200']
  if (!response || response.isRef()) throw new Error('expected concrete response')
  assertEquals(response.stackTrail.toJsonPointer(), '#/paths/~1users~1{id}/get/responses/200')

  const mediaType = response.content?.['application/json']
  if (!mediaType) throw new Error('expected application/json media type')
  assertEquals(
    mediaType.stackTrail.toJsonPointer(),
