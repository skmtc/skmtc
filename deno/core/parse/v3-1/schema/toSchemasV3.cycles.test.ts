/**
 * Cyclic composition (OpenAPI 3.1 tree) — the shapes that used to make `parse()` never return
 * (skmtc/skmtc#122), one test per case, each pinning the IR the parser now
 * produces. Mirror of the `parse/v3-0` file with a 3.1 header.
 *
  ...
  const { document, context } = parseSchemas({
    Account: {
      ...
      properties: {
        ...
        Children: { type: 'array', items: { type: 'object', anyOf: [ref('Account')] } }
      }
    }
  })
...
Deno.test('cycles - mutual allOf is refused rather than expanded', () => {
  const { document, context } = parseSchemas({
    ...
    B: { allOf: [ref('A'), { type: 'object', properties: { b: { type: 'string' } } }] }
  })
