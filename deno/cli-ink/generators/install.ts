...
import { Checkbox, Input, Select } from '../components/index.ts'
...
export const toInstallPrompt = async (skmtcRoot: SkmtcRoot, projectName: string) => {
  ...
  const generators: string[] = await Checkbox.prompt({
    ...
    options: availableGenerators
      ...
      .map(({ name }) => ({ label: `jsr:${name}`, value: `jsr:${name}` }))
  })
...
const createProject = async ({
  ...
  const name = await Input.prompt({
    message: "Let's create a new Skmtc project. What would you like to call it?"
  })
