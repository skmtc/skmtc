...
const rootConfig = JSON.parse(await Deno.readTextFile(join(denoDir, 'deno.json'))) as {
  workspace?: string[]
...
const isResolvable = (specifier: string): boolean => {
  ...
  return rootImports.some(key => specifier === key || specifier.startsWith(`${key}/`))
}
...
for (const block of blocks) {
  ...
  const specifiers = [...block.code.matchAll(importPattern)].map(match => match[1] ?? match[2])
  if (specifiers.length === 0 && !block.flags.includes('check')) {
    skipped.push({ block, reason: 'no imports (implicit fragment)' })
    continue
  }
  const unresolvable = specifiers.filter(specifier => !isResolvable(specifier))
...
while (remaining.length > 0) {
  ...
  const checkOutput =
    new TextDecoder().decode(check.stderr) + new TextDecoder().decode(check.stdout)
  ...
  if (attributed.size === 0) {
    console.error('deno check failed but no block file could be attributed — raw output:\n')
...
if (updateBaseline) {
  await Deno.writeTextFile(baselinePath, JSON.stringify(currentFailures, null, 2) + '\n')
}
...
try {
  ...
  console.log(
    `bootstrap: wrote ${currentFailures.length} failing block(s) to ${relative(
      denoDir,
      baselinePath
    )}`
  )
...
console.log(
  `\n${blocks.length} ts/tsx blocks across ${markdownFiles.length} files: ` +
    ...
    } baselined), ${skipped.length} skipped (${[...skipsByReason.entries()]
