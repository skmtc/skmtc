...
### Optional companions on the entry

Three entry fields refine how tooling treats the schema; none is
needed for generation. `supportsVariant: () => true` says the
generator accepts extra variants under the subject key.
`toEnrichmentDefaults` seeds default values from the schema for an
editor to persist. `v.title('Label')` on a field names it in a form.
See [entry factories](../../reference/api/entry-factories.md).
...
2. It reads the **raw umbrella** from the three storage spots, through
   `context.readEnrichment`, which records every read for the post-run audit:
   - `subject` — `readEnrichment([id, <subject…>, variant])` at the path shown
     in the [routing table](../../concepts/enrichments.md#the-routing-structure)
     for that projection-base kind.
   - `generator` — `readEnrichment([id, '_generator'])`.
   - `stack` — `readEnrichment(['_stack'])`.
3. Keys under `subject` and `generator` that the schema does not declare are
   reported as `UNKNOWN_ENRICHMENT_KEY` warnings. The `stack` bag is exempt:
   other generators own its other keys.
4. The `{ subject, generator, stack }` raw object is parsed **once** through the
   generator's composite `toEnrichmentSchema()` —
   `v.parse(config.toEnrichmentSchema(), raw)`. Because the composite schema is
   required, this parse is cast-free. A wrong-typed value throws here; the item
   is recorded as `error` in the manifest and the run continues.
5. The parsed umbrella is delivered as `settings.enrichments` to the Projection.
...
Unknown keys are stripped, and reported as `UNKNOWN_ENRICHMENT_KEY` on
`manifest.enrichmentWarnings`; missing optional keys arrive as `undefined`.
...
Declare the scope on the umbrella as well (`generator: generatorSchema`,
`stack: v.optional(stackSchema)`): the engine parses every projection's
umbrella against the same `_generator` / `_stack` values, and a scope left
`v.undefined()` rejects any value at that key. For `_stack` that holds for
every generator in the run, so a stack value can only be set once each of
them declares a `stack` member that accepts it.
