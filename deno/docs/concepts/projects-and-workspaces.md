...
├── bundle.js                    ← derived from worker.ts (if local generators)
├── .settings/
...
**Only present if the project has local (cloned or authored)
generators.** Remote-only projects skip the local bundle and use the
JSR-published version.
...
If the project has *only* installed generators (no clones), nothing
— there's no local bundle to update. The JSR-published bundle is
used.

If the project has at least one clone, `skmtc install` triggers a
post-install rebundle. The new generator gets bundled in alongside
the cloned ones.
