...
| **`ModelProjectionBase` / `OasOperationProjectionBase` / `GqlOperationProjectionBase`** | Specialised React.Component subclasses for `<UserType>`, `<EditForm>`, `<MutationHook>` | Three flavours of Projection, each with its own static-method contract (`toIdentifier`, `toExportPath`, …). Pick the right one based on what drives the Definition: a refName, an OAS operation, or a GQL operation. |
...
| **`Identifier`** | A name + an entity-type tag (`'variable'` or `'type'`) | Use `Identifier.createVariable('Foo')` or `Identifier.createType('Foo')`. The entity-type tag flows into import statements under `verbatimModuleSyntax`. |
...
| **`transform` (in an entry config)** | A `useEffect` body — runs per `(operation, variant)` pair | Return value is discarded for OAS; threaded as `acc` for GraphQL. Output happens via `register` / `insertOperation`, not the return. |
...
| **`toIdentifier` / `toExportPath`** | Pure functions producing `(name, file)` for a given operation | Must be pure — no `this`, no async, no env reads. The cross-generator cache depends on this. |
