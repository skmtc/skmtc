...
`export class MyProjection extends OasOperationProjectionBase ...`
in `mod.ts` makes the class available; it does not register the
class with the engine or arrange for it to be called.
