...
| **basePath** | `client.json#settings.basePath` | **Must match the consumer app's `@` alias root.** Both the on-disk root for generated files AND the alias root in the bundler's resolver. Generators emit `@/<subdir>/...` assuming this alignment. Absolute paths are rejected at `init`. |
...
When a required argument is missing in strict mode, the CLI emits a
...
| `agent-context` | Emit JSON dump of CLI surface + state | No args; always strict |
...
```jsonc
{
  ...
  "settings": {
    ...
    // Must be relative. Generators emit `@/<subdir>/...` paths
...
- `include === []` or undefined → no filter active; everything emits
  (backwards-compatible default)
- `include` mentions a generator → only matched items emit from that
  generator
...
a blanket-emit would produce dozens of files the team doesn't want.
...
**`--watch` and `--json` are mutually exclusive.** `--json` emits a
...
This emits forms only for the listed (path, method) pairs. Other
operations route through other generators normally. Other generators
not mentioned in `include` are unaffected — they continue emitting.
...
| Run Prettier in the pipeline | Don't — emit valid TS; consumer formats separately |
