...
export class MyLibNumber extends TsSnippet {
...
export class MyLibInteger extends TsSnippet {
  ...
  override toString(): string {
    // SLOT(integer): if the target has no integer kind, compose it
    // (e.g. number + integer constraint).
    return applyModifiers(`${LIB}.integer()`, this.modifiers)
  }
}

export class MyLibBoolean extends TsSnippet {
...
export class MyLibUnknown extends TsSnippet {
  type = 'unknown' as const

  constructor(
    { context, destinationPath, generatorKey, schema }: MyLibUnknownArgs,
  ) {
    super({ context, generatorKey, stackTrail: schema?.stackTrail.clone() })
...
export class MyLibVoid extends TsSnippet {
  type = 'void' as const

  constructor({ context, generatorKey, destinationPath }: MyLibVoidArgs) {
