 ...
 * its import.
 */
export class MyLibRef extends TsSnippet {
  type = 'ref' as const
  modifiers: Modifiers
  name: string
  terminal: boolean
  ...
  constructor(
    ...
    if (context.modelDepth[`${myLibEntry.id}:${refName}`] > 0) {
      ...
      context.modelDepth[`${myLibEntry.id}:${refName}`]++

      const settings = context.toModelContentSettings({
        refName,
        projection: MyLibProjection,
        variant: 'main',
      })
  ...
  override toString(): string {
    ...
    // SLOT(lazy): the target's deferred-reference form for recursive
    // cycles (zod: `z.lazy(() => x)`).
    return this.terminal ? `${LIB}.lazy(() => ${out})` : out
  }
}
