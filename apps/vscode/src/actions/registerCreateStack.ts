...
export const registerCreateStack = (store: ExtensionStore) => {
  return commands.registerCommand('skmtc-vscode.createStack', async () => {
    const name = await window.showInputBox({
      ...
      validateInput: value => {
        ...
        if (trimmed.length < 3) {
          return 'Stack name must be at least 3 characters long'
        }

        if (trimmed.length > 20) {
          return 'Stack name must be less than 20 characters long'
        }

        if (!/^[a-z0-9-]+$/.test(trimmed)) {
          return 'Stack name must only contain lowercase letters, numbers and hyphens'
        }

        if (trimmed.startsWith('-')) {
          return 'Stack name cannot start with a hyphen'
        }

        if (trimmed.endsWith('-')) {
          return 'Stack name cannot end with a hyphen'
        }
