...
export const toMilestonesNodes = (): MilestonesNode[] => {
  ...
  return [
    new MilestonesNode({
      nodeId: 'hasProject',
      nodeLabel: 'Create project',
      command: !withProject
        ? {
            title: 'Click here to create new project',
            command: 'skmtc-vscode.createProject',
          }
        : undefined,
...
const hasDeployment = () => {
  ...
  return Boolean(clientConfig?.serverName && clientConfig?.deploymentId);
};
