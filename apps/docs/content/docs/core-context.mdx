...
1. **Parse Phase**: Uses [ParseContext](./parse-context.mdx) to convert OpenAPI JSON into OAS objects
2. **Generate Phase**: Uses [GenerateContext](./generate-context.mdx) to transform OAS objects with generators  
3. **Render Phase**: Uses [RenderContext](./render-context.mdx) to format and prepare final artifacts
