...
exports.dump = dump;
const _error_js_1 = require("../_error.js");
const common = __importStar(require("../_utils.js"));
...
function encodeHex(character) {
    ...
    if (character <= 0xff) {
        ...
        throw new _error_js_1.YAMLError("code point within a string may not be greater than 0xFFFFFFFF");
    }
...
function writeScalar(state, string, level, iskey) {
    state.dump = (() => {
        ...
        switch (chooseScalarStyle(string, singleLineOnly, state.indent, lineWidth, testAmbiguity)) {
            ...
            default:
                throw new _error_js_1.YAMLError("impossible error: invalid scalar style");
        }
    })();
}
...
function writeBlockMapping(state, level, object, compact = false) {
    ...
    if (state.sortKeys === true) {
        ...
        // Something is wrong
        throw new _error_js_1.YAMLError("sortKeys must be a boolean or a function");
    }
        ...
        if ((type.instanceOf || type.predicate) &&
            ...
            if (type.represent) {
                ...
                if (_toString.call(type.represent) === "[object Function]") {
                    ...
                    throw new _error_js_1.YAMLError(`!<${type.tag}> tag resolver accepts not "${style}" style`);
                }
                state.dump = _result;
            }
            return true;
    ...
    if (duplicate && state.usedDuplicates[duplicateIndex]) {
        ...
        if (type === "[object Object]") {
            ...
            if (state.skipInvalid)
                return false;
            throw new _error_js_1.YAMLError(`unacceptable kind of an object to dump ${type}`);
        }
        if (state.tag !== null && state.tag !== "?") {
